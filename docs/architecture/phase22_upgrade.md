...
> **Symptom:** After fixing the database URL, the container stopped crashing but began hanging indefinitely. Hitting the endpoint via HTTPS returned a `502 Bad Gateway`. Running a deep `nc -vz ***REDACTED*** 5432` ping from the Droplet revealed 100% packet loss.
